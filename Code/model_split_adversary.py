...
#			self.batch_norm.training = False
			x = self.batch_norm(x)
#		self.rnn.traing = True
...
class NoiseClassifier(nn.Module):
	def __init__(self, rnn_type=nn.LSTM, rnn_hidden_size=768, nb_layers=1, bidirectional=True, nclasses = 8):
...
#		# rnns
		...
		for x in range(self.nb_layers):
			...
			rnns.append(('%d' % (x), rnn))
		self.noise_rnns = nn.Sequential(OrderedDict(rnns))
		# fc layers
		self.noise_fc = nn.Sequential(
                        nn.BatchNorm1d(rnn_hidden_size),
                        nn.Linear(rnn_hidden_size, self.fc_hidden_size),
			nn.ReLU(),
...
#				continue
#			pdb.set_trace()
			#rnn_path = os.path.join(rep_path, 'rnn_{}'.format(str(count)))
	...
	def serialize(model, optimizer=None, epoch=None, iteration=None, loss_results=None,
		...
		if optimizer is not None:
			if isinstance(optimizer, list):
				...
				package['optim_dict'] = [optimizer.state_dict() for optimizer in optimizers]
			else:
